...
public abstract class Grid
{
    ...
    public Image ToPng(int cellSize = 10)
    {
        ...
        foreach (var cell in EachCell())
        {
            ...
            if (cell.North == null)
                mazeImage.Mutate(x => x.DrawLine(wall, new Point(x1, y1), new Point(x2, y1)));
            if (cell.West == null)
                mazeImage.Mutate(x => x.DrawLine(wall, new Point(x1, y1), new Point(x1, y2)));
            if (!cell.IsLinked(cell.East))
                mazeImage.Mutate(x => x.DrawLine(wall, new Point(x2, y1), new Point(x2, y2)));
            if (!cell.IsLinked(cell.South))
                mazeImage.Mutate(x => x.DrawLine(wall, new Point(x1, y2), new Point(x2, y2)));
        }
