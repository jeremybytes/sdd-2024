...
You should have 6 replacements in ScheduleHelper.  
...
11. The code can be shorted using the Null Coalescing operator (??).  
...
12. The code can be further shorted by combining the assignment and null coalescing operator.  
...
13. The code can be further shortened by combining the assignment and return lines.  
...
14. The property get/set can also be updated to use expression-bodied members. Here is the final property:  
...
15. Update the static "Now" method to use the new property.  
...
The tests use a 3 part naming scheme. The first part is the unit under test (ScheduleItems). The second is the action run (OnCreation). The third is the expected result (IsPopulated). So the name of this test tells us that we expect the ScheduleItems to be populated when the Schedule class is created.  

> ScheduleItems is not a separate property of the Schedule class. The Schedule class itself is a List&lt;ScheduleItem&gt;. So the class itself is a collection of ScheduleItems. For example, if we want to know the number of ScheduleItems, we use Schedule.Count (a property on List).

In addition, the tests use Arrange / Act / Assert for organization. The Arrange section has any test setup code. The Act section is where the action is performed. The Assert section is where the results are checked. In some instances, such as this test, the Arrange and Act sections are combined (since this covers what happens at object creation).  
...
For this set of test, create a fake sunset provider.  
...
> NUnit asserts uses an "Assert.That" syntax. The first parameter is the thing that we are testing (the count of items in the schedule). The second parameter is the condition (greater than 0). There are a number of helper items in NUnit (like "Is") that is used here. "Is.EqualTo(expectedValue)" is a common condition, but there is quite a bit of flexibility.  
...
19. The "Assert" is the same as the previous test: to make sure all items are now in the future:
...
22. Create the schedule (as in previous tests) and add a new item.
...
27. Review the last test in the ScheduleTests class.
...
14. Mocking framework by creating in-memory objects that you configure with the behavior that you want. The advantage of using a mock over a fake is that you only need to provide behavior that you need. For a complex object, this may mean only filling a couple of properties or methods rather than creating an entire class.  
...
> DurationFromNow, IsInPast, and IsInFuture are all interesting candidate for extension methods. In order to do this, the methods would need to be moved to a static class, and the "this" keyword added before the parameters. Since it relies on the TimeProvider, some other methods would need to be moved around as well.  
...
In addition to fake objects, you also used a mocking framework (Moq) to create an in-memory test object. These are good particularly for complex objects because you only need to configure the methods, property, and other members that you care about.  
