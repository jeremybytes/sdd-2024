...
5. At the top of the InitializeHosueController method, create an IKernel (the Ninject container type) and initialize it to a new StandardKernel (the default Ninjection container instance).  
...
> Note: It is generally not good to bind to a generic type like "string" because there are often multiple strings needed. In this case, it will get you headed in the right direction, and you can deal with fixing it in a later step.  
...
Sunset Tomorrow: 5/4/2024 12:28:31 PM
Device 5: On
5/3/2024 10:33:05 AM - Device: 5, Command: On
Device 5: Off
5/3/2024 10:33:05 AM - Device: 5, Command: Off
...
>This binding is only for to configure the property setting. As you saw earlier, the Ninject container can still create the HouseController object without this configuration.
...
> Note: If you are in an ASP.NET Core project (or other application that uses the application builder), you instead set a qualifier based on the environment. 
...
Instead, the Ninject container should provide us with this instance. But there's a problem: the parameters (latitude and longitude) are both of type "double", so there is no quick way to configure that in the container (like you did with the filename string).  
...
7. Create a Ninject container binding for the LatLongLocation type.  
...
You also saw how to use custom types for easier configuration. Containers are good at mapping types to values. If you have a generic type (such as a string), things can get ambiguous, particularly if more than one object has a dependency on a string.  More robust types can help with that.
...
This particular application does not have many features, so we would not get much benefit from this. But in a larger application with screens, modules, and functions, we can let the container deal with what items to keep around, which to discard, and which to maintain at a scope level.  
